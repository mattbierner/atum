        ...
        return bind(q, function(y) {
            ...
            case completion.BreakCompletion.type:
             ...
             case completion.ThrowCompletion.type:
                 return completeThrow(y.value, x);
            }
            return just(y);
        });
    });
...
var liftExpression = function(expr) {
    return function(ctx, k) {
        return expr(ctx,
            function(x, ctx) { return cont(x && x instanceof completion.Completion ? just(x) : completeNormal(x), [ctx, k]); },
            function(x, ctx) { return cont(x && x instanceof completion.Completion ? just(x) : completeThrow(x), [ctx, k]); });
    };
};
