...
var parse = function(text) {
    var ast;
    try {
        return compute.just(parser.parseStream(lexer.lex(text)));
    } catch (e) {
    ...
    return program.bindStatement(body, function(x) {
        if (x instanceof completion.Completion) {
            ...
            case completion.ReturnCompletion.type:
                return error.syntaxError("Return statements are only valid inside of functions");
...
var evaluateAst = function(ast) {
    return compute.bind(
        compute.extract(function(ctx) { return ctx.semantics; }),
        function(ctxSemantics) {
            return evaluate(
                extract(
                    (ctxSemantics || semantics).sourceElements(ast.body)));
        });
};
 ...
 */
var evaluateText = compute.compose(
    parse,
    evaluateAst);
...
var evaluateUrlFile = function(name) {
    return compute.bind(
        compute.liftIO(io.getFile(name)),
        evaluateModuleText);
};
