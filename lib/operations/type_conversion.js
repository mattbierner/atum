...
var toNumber = compute.compose(
    ...
    compute.from(type_conversion.toNumber));
...
var toInteger = compute.compose(
    toNumber,
    compute.from(type_conversion.toInteger));
...
var toInt32 = compute.compose(
    toNumber,
    compute.from(type_conversion.toInt32));
...
var toUint32 = compute.compose(
    toNumber,
    compute.from(type_conversion.toUint32));
...
var toString =  compute.compose(
    ...
    compute.from(type_conversion.toString));
...
var toObject = function(input) {
    return value_reference.dereference(input, function(v, ref) {
        ...
        case type.UNDEFINED:
        case type.NULL:
            return error.typeError(v, ' cannot be converted to an object');
