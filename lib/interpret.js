 ...
 */
define(['ecma/ast/node', 'ecma/ast/declaration',
        'atum/compute',
        'atum/value/undef', 'atum/value/boolean',
...
// Statement
    ...
    case 'WhileStatement':
        return semantics.whileStatement(
            (node.test ? evaluate(node.test) : compute.always(new boolean.Boolean(true))),
            evaluate(node.body));
    ...
    case 'ForStatement':
        return semantics.forStatement(
            ...
            (node.body ? evaluate(node.body) : semantics.emptyStatement()));
...
var interpret = function(root) {
    return eval(
        new execution_context.ExecutionContext(
            execution_context.GLOBAL,
            false,
