...
/**
 * TODO: Better solution for stopping iteration.
 */
 ...
 * 
 */
var inspect = function(p, ctx, ok, err) {
    return p(ctx, ok, err);
};
...
/**
 * Interprets AST 'root' in an new context
 * 
...
var eval = function(comp) {
    return evalContext(comp, new compute.ComputeContext({}, null));
};

var evaluateProgram = function(root) {
    return compute.next(global.initialize(), evaluate(root));
};
...
var interpret = (function(){
    ...
    return function(root) {
        return eval(extract(evaluateProgram(root)));
    };
}());
