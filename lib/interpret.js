...
ThrowCompletion.prototype.type = "Throw";
...
var variableDeclaration = function(declarations) {
    return reduce.call(declarations, next, compute.always(null));
};

/**
 * 
 */
var variableDeclarator = function(identifier, Initialiser) {
    return compute.bind(identifier, function(id) {
        return compute
    });
};
...
// Value
    case 'Identifier':
        return function(ctx, ok, err) {
            return ok(environment.getIdentifierReference(ctx.lexicalEnvironment, node.name, ctx.strict));
        };
