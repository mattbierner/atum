...
// Statement
    ...
    case 'TryStatement':
        return semantics.tryStatement(
            evaluate(node.block),
            (node.handler ? node.handler.param.name : null),
            (node.handler ? evaluate(node.handler.body) : null),
            (node.finalizer ? evaluate(node.finalizer) : null));
...
// Declarations
    ...
    case 'VariableDeclarator':
        return (node.init ?
            semantics.variableDeclarator(node.id.name, evaluate(node.init)) :
            semantics.emptyStatement());
...
var interpret = function(root) {
    return eval(compute.next(global.initialize(), evaluate(root)));
};
