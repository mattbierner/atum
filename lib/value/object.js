...
AtumObject.prototype.get = function(ref, name) {
    return compute.bind(this.getProperty(ref, name), function(prop) {
        if (prop) {
            if (prop.get)
                return func.call(compute.just(prop.get), prop.get.getValue(), compute.just(ref), compute.enumeration());
            else if (prop.value)
...
AtumObject.prototype.set = function(ref, name, value) {
    ...
    return compute.bind(this.getProperty(ref, name), function(prop) {
        ...
        return (prop.set ?
            func.call(compute.just(prop.set), prop.set.getValue(), compute.just(ref), compute.enumeration(compute.just(value))) :
