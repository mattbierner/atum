...
var sourceElements = function(strict, declarations, statements) {
    var body = statement.statementNext(
        (declarations ? statement.statementList(declarations) : compute.empty),
        statement.statementList(fun.map(sourceElement, statements)));
    return compute.just(strict ?
...
var programBody = function(body) {
    return compute.just(
        program_computations.bindStatement(body, extract));
};
