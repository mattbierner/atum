...
var statementList = function(statements) {
    return fun.reduce(statementNext, statement.empty, statements);
};//fun.curry(fun.reduce, statementNext, statement.empty);
...
var _iter = function(test, pre, body, post) {
    pre = statement.liftExpression(pre);
 ...
 * While statement
 * 
 * @param test Expression of the loop conditional to stops iteration if falsy.
 * @param body Statement-like computation loop body.
 */
var whileStatement = fun.placeholder(_iter,
    fun._,
    compute.empty,
    fun._,
    compute.empty);
