...
var statementList = function(statements) {
    return (statements.length ?
        reduce(statements, statement.next) :
        emptyStatement);
};
...
var throwStatement = function(argument) {
    return compute.Computation('Throw Statement',
        statement.liftExpression(complete.completeThrowFrom(
             debug_operations.debuggable(
                 internal_reference.getFrom(argument)))));
};
...
var forStatement = function(init, test, update, body) {
    ...
    var iter = ifStatement(test,
        statement.bind(body, function(x) {
            if (x instanceof completion.AbruptCompletion) {
                switch (x.type) {
                case completion.ContinueCompletion.type:
                    return statement.liftExpression(compute.next(
                        update,
                        statementNext(
    ...
    return statement.liftExpression(compute.next(init, iter));
};
                    ...
                    return (function iter(i) {
                        ...
                        return statement.bindExpression(internal_reference.setValue(lhs, string.create(props[i])), function() {
                            return statement.bind(body, function(result) {
                                switch (result.type) {
                                case completion.NormalCompletion.type:      return statementNext(compute.just(result), iter(i + 1));
                                case completion.ContinueCompletion.type:    return statementNext(complete.completeNormal(result.value), iter(i + 1));
                                case completion.BreakCompletion.type:       return complete.completeNormal(result.value);
                                default:                                    return compute.just(result);
                                }
                            });
                        });
                    }(0));
