...
var attempt = function(p) {
    return compute.bindError(p, function(e) {
        return compute.just(new completion.ThrowCompletion(e));
    });
};
...
var expressionStatement = function(expr) {
    return completion_semantics.createNormalCompletion(internal_reference.getValue(attempt(expr)));
};
...
var doWhileStatement = function(body, test) {
    return compute.bind(body, function(x) {
        if (x instanceof completion.AbruptCompletion) {
            switch (x.type) {
            case completion.BreakCompletion.type:       return compute.just(x.value);
    ...
    var mergeCompletion = function(a, b) {
        return compute.bind(a, function(x) {
            return compute.bind(b, function(y) {
                return compute.just(!(y instanceof completion.AbruptCompletion) ? x : y);
            });
        });
    };
                        ...
                        return compute.bind(
                            ...
                            function(e) {
                                return environment_semantics.withEnvironment(e,
                                    compute.next(
                                        environment_semantics.putMutableBinding(handlerId, false, compute.just(error.value)),
                                        statementNext((error.previous ? compute.just(error.previous) : emptyStatement()), handler)));
                            });
                        });
                }
                return compute.just(error);
            }),
            finalizer);
    };
}());
