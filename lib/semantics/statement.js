...
/**
 * Capture the current environment, perform statement `p`, then restore old environment.
 */
 ...
 */
var emptyStatement = compute.just(statement.empty);
 ...
 */
var expressionStatement = fun.compose(compute.just,fun.compose(
 ...
 */
var blockStatement = fun.compose(compute.just, statementList);
 ...
 */
var withStatement = fun.compose(compute.just, function(expr, body) {
    return statement.bindExpression(
 ...
 */
var returnStatement = fun.compose(compute.just, fun.compose(
    debug_operations.debuggable,
    fun.compose(
 ...
 */
var throwStatement = fun.compose(compute.just, fun.compose(
    debug_operations.debuggable,
    fun.compose(
 ...
 * @param {String} [label]
 */
var breakStatement = fun.compose(compute.just, fun.compose(
    debug_operations.debuggable,
 ...
 * @param {String} [label]
 */
var continueStatement = fun.compose(compute.just,fun.compose(
    debug_operations.debuggable,
 ...
 *    value.
 */
var ifStatement = fun.compose(compute.just, function(test, consequent, alternate) {
    return statement.bindExpression(
        debug_operations.debuggable(
...
var _switchCase = function(discriminant, test, consequent, alternate) {
    return compute.bounce(ifStatement(
 ...
 * @TODO: ugly
 */
var switchStatement = fun.compose(compute.just,function(discriminant, preCases, defaultCase, postCases) {
    var behaviors = fun.concat(
...
var _iter = function(test, pre, body, post) {
    ...
    var iter = compute.bounce(ifStatement(test,
        statement.next(
 ...
 */
var whileStatement = fun.compose(compute.just, function(test, body) {
    return _iter(
        test,
        compute.empty,
        body,
 ...
 */
var doWhileStatement = fun.compose(compute.just, function(body, test) {
    return statement.bind(body, function(x) {
        ...
        return compute.bounce(whileStatement(test, body));
    });
});
 ...
 */
var forStatement = fun.compose(compute.just, function(init, test, update, body) {
    return statement.next(
 ...
 */
var forInStatement = fun.compose(compute.just, function(lhs, rhs, body) {
    return statement.bindExpression(
 ...
 * @param body Loop body statement.
 */
var forVarInStatement = fun.compose(compute.just, function(id, rhs, body) {
    return statement.bindExpression(
        environment_reference.create(id),
        function(ref) {
            return compute.bounce(forInStatement(
                compute.just(ref),
                rhs,
                body));
    });
...
var tryCatchFinallyStatement = (function(){
    ...
    return fun.compose(compute.just, function(body, handlerId, handler, finalizer) {
            ...
            return environmentBlock(
                compute.next(
                    ...
                    compute.next(
                        ...
                        statementNext((previous ? compute.just(previous) : statement.empty), handler))));
        };
