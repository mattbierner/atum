...
var subtractOperator = function(left, right) {
    return number.subtract(
        internal_reference.dereferenceFrom(left, type_conversion.toNumber),
        internal_reference.dereferenceFrom(right, type_conversion.toNumber));
};
...
var leftShiftOperator = function(left, right) {
    return number.leftShift(
        internal_reference.dereferenceFrom(left, type_conversion.toInt32),
        internal_reference.dereferenceFrom(right, type_conversion.toUint32));
};
...
var bitwiseAndOperator = function(left, right) {
    return number.unsignedRightShift(
        internal_reference.dereferenceFrom(left, type_conversion.toInt32),
        internal_reference.dereferenceFrom(right, type_conversion.toInt32));
};
...
var logicalNotOperator = function(argument) {
    return compute.Computation('Logical Not',
        boolean.logicalNot(
            internal_reference.dereferenceFrom(argument, type_conversion.toBoolean)));
};
    ...
    return compute.Computation('Typeof operator',
        compute.bind(argument, function(val) {
            return (val && val.isUnresolvable ?
                ...
                string.from(
                    object.typeofObject(
                        internal_reference.getValue(compute.just(val)))));
        }));
};
    ...
    return compute.Computation('Member Expression',
        compute.binds(
            compute.enumeration(
                ...
                base,
                value_reference.getFrom(internal_reference.getValue(base)),
