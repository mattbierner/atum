...
var _prefixUpdate = function(op) {
    return function(arg) {
        return compute.bind(arg, function(x) {
            ...
            return op(arg, internal_reference.dereference(x, type_conversion.toNumber));
        });
    };
};
...
var _increment = function(expr, val) {
    return compute.bind(number.increment(val), function(num) {
        return internal_reference.setValue(expr, num);
    });
};

var _decrement = function(expr, val) {
    return compute.bind(number.decrement(val), function(num) {
        return internal_reference.setValue(expr, num);
    });
};
