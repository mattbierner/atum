/**
 * @fileOverview Semantics for dealing with references.
 */
define(['atum/compute',
        'atum/reference',
        'atum/value/type_conversion',
        'atum/semantics/environment'],
function(compute,
        reference,
        type_conversion,
        environment){

/* Computations
 ******************************************************************************/
/**
 * Create a computation that dereferences the result of computation 'p'.
 */
var getValue = function(p) {
    return compute.bind(p, function(x) {
        if (!(x instanceof reference.Reference)) {
            return compute.always(x);
        }
        if (x.isUnresolvable) {
            return compute.never(new reference.ReferenceError(x.name));
        }
        
        var base = x.base,
            name = x.name;
        if (x.isProperty) {
            return (x.hasPrimitiveBase ?
                compute.bind(
                    type_conversion.toObject(base),
                    function(o) {
                        return compute.always(o[p]);
                    }) :
                compute.always(base[name]));
        } else { // Must be an environment record.
            return compute.always(base.getBindingValue(name, x.isStrict));
        }
    });
};

/**
 * 
 */
var putValue = function(v, w) {
    return function(ctx, ok, err) {
        if (!v instanceof reference.Reference) {
            return err(new reference.ReferenceError());
        }
        var base = v.base;
        if (v.isUnresolvable) {
            if (v.isStrict) {
                return err(new reference.ReferenceError("E"));
            } else {
                return environment.setMutableBinding(v.name, w)(ctx, ok, err);
            }
        } else if (v.isProperty) {
          //  if (v.hasPrimitiveBase )
        } else {
            return environment.setMutableBinding(v.name, w, v.isStrict)(ctx, ok, err);
        }
    };
};


return {
    'getValue': getValue,
    'putValue': putValue
};

});