...
var callExpression = function(callee, args) {
    return compute.binda(
        ...
        function(ref, func) {
            return compute.binda(
                compute.sequence.apply(undefined, args),
                function(/*...*/) {
                    if (value.type(func) !== 'object' || !value.isCallable(func)) {
                        return compute.never("TypeError");
                    }
                    if (ref instanceof reference.Reference) {
                        return func.call(ref.isProperty ? ref.base : ref.base.implicitThisValue(), arguments);
                    }
                    return func.call(undef.create(), arguments);
                });
        });
};
...
var memberExpression = function(base, property) {
    return compute.binda(
        ...
        function(ctx, baseValue, name) {
            return compute.always(new reference.ValueReference(name.value, baseValue, ctx.strict));
        });
};
...
var initVariableDeclarator = function(id, init) {
    return compute.bind(
        compute.context,
        function(ctx) {
            return environment_semantics.setMutableBinding(id, ctx.strict, init);
        });
};
