    ...
    var iter = function(v) {
        return ifStatement(type_conversion.toBoolean(reference_semantics.getValue(test)),
            compute.bind(body, function(x) {
                if (x instanceof completion.Completion) {
                    switch (x.type) {
                    case 'continue':    return iter(x.value === null ? v : compute.always(x.value));
                    case 'break':       return compute.always(x.value);
                    }
                    return compute.always(x);
                }
                return iter(compute.always(x));
            }),
            v);
    };
    return iter(undef_semantics.create());
};
    ...
    return compute.next(
        ...
        whileStatement(
            ...
            compute.bind(
                body,
                function(x) {
                    return compute.next(update, compute.always(x));
                })));
};
