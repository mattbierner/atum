...
var expressionStatement = function(expr) {
    return completion_semantics.createNormalCompletion(reference_semantics.tryGetValue(expr));
};
...
var ifStatement = function(test, consequent, alternate) {
    return compute.bind(
        type_conversion.toBoolean(reference_semantics.tryGetValue(test)),
        function(x) {
...
var returnStatement = function(argument) {
    return compute.Computation('Return Statement',
        completion_semantics.createReturnCompletion(reference_semantics.tryGetValue(argument)));
};
...
var throwStatement = function(argument) {
    return compute.Computation('Throw Statement',
        compute.bind(
            reference_semantics.tryGetValue(argument),
            compute.never));
};
...
var _switchCase = function(discriminant, test, consequent, alternate) {
    return ifStatement(strictEqualOperator(test, discriminant), consequent, alternate);
};
...
var addOperator = function(left, right) {
    return compute.binary(
        type_conversion.toPrimitive(reference_semantics.tryGetValue(left)),
        type_conversion.toPrimitive(reference_semantics.tryGetValue(right)),
...
var subtractOperator = function(left, right) {
    return number.subtract(
        type_conversion.toNumber(reference_semantics.tryGetValue(left)),
        type_conversion.toNumber(reference_semantics.tryGetValue(right)));
};
...
var leftShiftOperator = function(left, right) {
    return number.leftShift(
        type_conversion.toInt32(reference_semantics.tryGetValue(left)),
        type_conversion.toUint32(reference_semantics.tryGetValue(right)));
};
...
var bitwiseAndOperator = function(left, right) {
    return number.unsignedRightShift(
        type_conversion.toInt32(reference_semantics.tryGetValue(left)),
        type_conversion.toInt32(reference_semantics.tryGetValue(right)));
};
...
var equalOperator = function(left, right) {
    return compare.equal(
        reference_semantics.tryGetValue(left),
        reference_semantics.tryGetValue(right));
};
...
var unaryPlusOperator = function(argument) {
    return type_conversion.toNumber(reference_semantics.tryGetValue(argument));
};
...
var unaryMinusOperator = function(argument) {
    return number.negate(type_conversion.toNumber(reference_semantics.tryGetValue(argument)));
};
...
var logicalNotOperator = function(argument) {
    return boolean.logicalNot(type_conversion.toBoolean(reference_semantics.tryGetValue(argument)));
};

var bitwiseNotOperator = function(argument) {
    return number.bitwiseNot(type_conversion.toInt32(reference_semantics.tryGetValue(argument)));
};
...
var voidOperator = function(argument) {
    return compute.next(reference_semantics.tryGetValue(argument),
        undef_semantics.create());
};
...
var _prefixUpdate = function(op) {
    return function(arg) {
        return compute.bind(arg, function(x) {
            ...
            return op(arg, type_conversion.toNumber(reference_semantics.tryGetValue(arg)));
        });
    };
};
...
var _postfixUpdate = function(op) {
    return function(arg) {
        return compute.bind(arg, function(x) {
            ...
            return compute.bind(
                type_conversion.toNumber(reference_semantics.tryGetValue(arg)),
                function(x) {
...
var logicalOr = function(left, right) {
    return compute.bind(
        type_conversion.toBoolean(reference_semantics.tryGetValue(left)),
        function(result) {
...
var callExpression = function(callee, args) {
    return compute.Computation('Call Expression',
        compute.binds(
            compute.sequence(
                callee,
                value_reference.get(reference_semantics.tryGetValue(callee))),
            function(ref, func) {
                return compute.binds(
                    compute.sequence(
                        compute.sequence.apply(undefined, args.map(reference_semantics.tryGetValue)),
    ...
    return compute.Computation('Member Expression',
        compute.binds(
            compute.sequence(
                compute.getContext(),
                base,
                value_reference.get(reference_semantics.tryGetValue(base)),
                type_conversion.toString(reference_semantics.tryGetValue(property))),
...
var conditionalExpression = function(test, consequent, alternate) {
    return compute.Computation('Conditional Expression',
        compute.bind(
            type_conversion.toBoolean(reference_semantics.tryGetValue(test)),
            function(x) {
                return reference_semantics.tryGetValue(x.value ? consequent : alternate);
            }));
};
...
var sequenceExpression = function(expressions) {
    return compute.Computation('Sequence Expression',
        reduce(expressions, function(p, c) {
            return compute.next(p, reference_semantics.tryGetValue(c));
        }));
};
...
var objectLiteral = function(properties) {
    return reduce(Object.keys(properties), function(p, key) {
        ...
        return object.defineProperty(p, key, {
            'value': prop.value ? reference_semantics.tryGetValue(prop.value) : null,
            'get': prop.get ? reference_semantics.tryGetValue(prop.get) : null,
            'set': prop.set ? reference_semantics.tryGetValue(prop.set) : null
        });
...
var variableInitDeclarator = function(id, init) {
    return compute.bind(
        compute.getContext(),
        function(ctx) {
            return environment_semantics.putMutableBinding(id, ctx.strict, reference_semantics.tryGetValue(init));
        });
};
...
var variableDeclarator = function(id) {
    return variableInitDeclarator(id, undef_semantics.create());
};
