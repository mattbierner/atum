...
var forStatement = function(loc, init, test, update, body) {
    return statement_semantics.forStatement(
        (init ? mapSemantics(init) : statement_semantics.emptyStatement()),
        (test ? mapSemantics(test) : value_semantics.booleanLiteral(true)),
        (update ? mapSemantics(update) : statement_semantics.emptyStatement()),
        mapSemantics(body));
};
...
var unaryExpression = function(loc, operator, argument) {
    ...
    default: return compute.error("Unknown Unary Operator:" + operator);
    }
};
...
var updateExpression = function(loc, operator, prefix, argument) {
    ...
    default: return compute.error("Unknown Update Operator:" + operator);
    }
};
...
var binaryExpression = function(loc, operator, left, right) {
    ...
    default: return compute.error("Unknown Binary Operator:" + operator);
    }
};
...
var logicalExpression = function(loc, operator, left, right) {
    ...
    default:    return compute.error("Unknown Logical Operator:" + operator);
    }
};
...
var assignmentExpression = function(loc, operator, left, right) {
    ...
    default:    return compute.error("Unknown Assignment Operator:" + operator);
    }
};
...
var arrayExpression = function(loc, elements) {
    throw "Array Expression";
};

var objectExpression = function(loc, properties) {
    return expression_semantics.objectLiteral(reduce(properties, function(p, c) {
        var key = (c.key.name === undefined ? c.key.value : c.key.name),
            value = mapSemantics(c.value);
        if (!p[key]) {
            p[key] = {
                'writable': true,
                'enumerable': true,
...
var functionExpression = function(loc, id, params, body) {
    return expression_semantics.functionExpression(
        (id === null ? null : id.name),
        map(params, function(x) {
            return x.name;
        }),
...
var literal = function(loc, kind, value) {
    ...
    default:        return compute.error("Unknown Literal of kind: " + kind);
    }
};
