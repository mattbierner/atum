...
define(['atum/compute',
        ...
        'atum/semantics/environment', 'atum/semantics/number',
...
function(compute,
        ...
        environment_semantics, number_semantics,
 ...
 ******************************************************************************/
var prefixIncrement = function(argument) {
    return compute.bind(argument, function(expr) {
        if (expr instanceof reference.Reference &&
            expr.strict &&
            expr.base instanceof environment.EnvironmentRecord &&
            (expr.name === 'eval' || expr.name === 'arguments')) {
            return compute.never("SyntaxError");
        }
        return compute.bind(
            type_conversion.toNumber(reference_semantics.getValue(argument)),
            function(oldValue) {
                var newValue = number.add(oldValue, new number.Number(1));
                return compute.next(
                    reference_semantics.putValue(expr, newValue),
                    compute.always(newValue));
            });
    });
};
