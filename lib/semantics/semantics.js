...
var declarations = function(node) {
        var empty = compute.bind(program_fragment.emptyStatement, fun.compose(compute.just, compute.dispatch));
    ...
    case 'BlockStatement':
        return compute.bind(
            ...
            fun.compose(compute.just, program_fragment.statementList));
...
var mapSemantics = (function(){
    ...
    // Statement
        ...
        case 'BreakStatement':
            return program_fragment.breakStatement(
                (node.label ? node.label.name : null));
        
        case 'ContinueStatement':
            return program_fragment.continueStatement(
                (node.label ? node.label.name : null));
    ...
    // Function
        ...
        case 'FunctionDeclaration':
             ...
             * so this is a noop.
             */
            return program_fragment.emptyStatement;
...
var emptyStatement = fun.constant(program_fragment.emptyStatement);
...
var blockStatement = function(elems) {
    return compute.bind(
        compute.enumerationa(fun.map(mapSemantics, elems)),
        program_fragment.blockStatement);
};

var expressionStatement = fun.compose(
    program_fragment.expressionStatement,
    mapSemantics);
...
var ifStatement = function(test, consequent, alternate) {
    return compute.binary(
        ...
        (alternate ?
            ...
            program_fragment.emptyStatement),
        function(consequent, alternate) {
            return program_fragment.ifStatement(
                mapSemantics(test),
                consequent,
                alternate);
    });
...
var returnStatement = fun.compose(
    program_fragment.returnStatement,
    mapSemantics);

var throwStatement = fun.compose(
    program_fragment.throwStatement,
    mapSemantics);
...
var withStatement = function(object, body) {
    return compute.bind(
        mapSemantics(body),
        function(body) {
            return program_fragment.withStatement(
                mapSemantics(object),
                body);
        });
};
...
var switchStatement = function(discriminant, cases) {
    ...
    return compute.binds(
        ...
        function(pre, def, post) {
            return program_fragment.switchStatement(
...
var tryStatement = function(block, handlerId, handler, finalizer) {
    return compute.binds(
        compute.enumeration(
            mapSemantics(block),
            mapSemantics(handler),
            mapSemantics(finalizer)),
        function(body, h, finalizer) {
            return program_fragment.tryStatement(body, handlerId, h, finalizer);
        });
};
...
var whileStatement = function(test, body) {
    return compute.bind(
        mapSemantics(body),
        function(body) {
            return program_fragment.whileStatement(
                mapSemantics(test),
                body);
        });
};
...
var doWhileStatement = function(body, test) {
    return compute.bind(
        mapSemantics(body),
        function(body) {
            return program_fragment.doWhileStatement(
                body,
                mapSemantics(test));
        });
};
...
var forStatement = function(init, test, update, body) {
    return compute.bind(
        mapSemantics(body),
        fun.curry(program_fragment.forStatement,
            mapSemantics(init),
            mapSemantics(test),
            mapSemantics(update)));
};
...
var forInStatement = function(left, right, body) {
    if (left.type === 'VariableDeclaration') {
        return compute.bind(
            mapSemantics(body),
            fun.curry(program_fragment.forVarInStatement,
                    left.declarations[0].id.name,
                    mapSemantics(right)));
    }
    return compute.bind(
        mapSemantics(body),
        fun.curry(program_fragment.forInStatement,
            mapSemantics(left),
            mapSemantics(right)));
};
