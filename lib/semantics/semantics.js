...
var statementList = function(statements) {
    var impl = function(val, statements) {
            ...
            compute.bind(
                ...
                function(x) {
                    if (x instanceof completion.Completion) {
                        switch (x.type) {
                        case 'normal':
                            return impl(completion_semantics.createNormalCompletion(x.value === null ? val : compute.always(x.value)), statements.slice(1));
                        case 'continue':
                            if (x.value === null)
                                return completion_semantics.createContinueCompletion(x.target, val);
                            break;
                        case 'break':
                            if (x.value === null)
                                return completion_semantics.createBreakCompletion(x.target, val);
                            break;
                        }
                        return compute.always(x);
                    }
                    return impl(compute.always(x), statements.slice(1));
                }));
    };
    return impl(compute.always(null), statements);
};
            ...
            compute.bind(body, function(x) {
                if (x instanceof completion.Completion) {
                    ...
                    case 'continue':
                        return compute.next(
                            update,
                            iter(x.value === null ? v : compute.always(x.value)));
                    case 'break':
                        return (x.value === null ? v : compute.always(x.value));
                    default:
                        return compute.always(x);
                    }
                }
                return compute.next(update, iter(compute.always(x)));
            }),
            v);
    };
    return compute.next(
        init,
        iter(undef_semantics.create()));
};
...
/* 
 ******************************************************************************/
var tryStatement = function(body, handlerId, handler, finalizer) {
    return (handlerId ?
        compute.bindError(body, function(error) {
            var h = compute.bind(environment_semantics.getEnvironment(), function(env) {
                return compute.bind(
                    (new iref.Iref()).set(compute.always(new environment.DeclarativeLexicalEnvironment(env))),
                    function(catchEnv) {
                        return compute.next(environment_semantics.setEnvironment(catchEnv),
                            compute.next(
                                environment_semantics.putMutableBinding(handlerId, false, compute.always(error)),
                                compute.then(handler,
                                    environment_semantics.setEnvironment(env))));
                    });
            });
            return (finalizer ? 
                compute.next(h, finalizer) :
                h);
        }) :
        compute.bindError(body, function(x) {
            return (finalizer ?
                compute.next(finalizer, compute.never(x)) :
                compute.never(x));
        }));
};
