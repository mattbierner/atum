...
var forStatement = function(init, test, update, body) {
    var iter = function(v) {
        return ifStatement(type_conversion.toBoolean(reference_semantics.getValue(test)),
            compute.bind(body, function(x) {
                if (x instanceof completion.Completion) {
                    switch (x.type) {
                    case 'continue':    return compute.next(update, iter(x.value === null ? v : compute.always(x.value)));
...
/**
 * @TODO if break in first iter?
 */
var doWhileStatement = function(body, test) {
    return forStatement(body, test, emptyStatement, body);
};
