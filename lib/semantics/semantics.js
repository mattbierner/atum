    ...
    return compute.bind(p, function(x) {
        ...
        return compute.bind(q, function(y) {
            if (y instanceof completion.Completion) {
                switch (y.type) {
                case 'normal':
                    return completion_semantics.createNormalCompletion(compute.always(y.value === null ? x.value : y.value));
                 case 'break':
                    return completion_semantics.createBreakCompletion(y.target, compute.always(y.value === null ? x.value : y.value));
                 case 'continue':
                    return completion_semantics.createContinueCompletion(y.target, compute.always(y.value === null ? x.value : y.value));
                }
            }
...
var statementList = function(statements) {
    return (statements.length ?
        reduceRight(statements, function(p, c) { return statementNext(c, p); }) :
        emptyStatement());
};
...
var tryStatement = function(body, handlerId, handler, finalizer) {
    return (handlerId ?
        compute.bindError(body, function(error) {
            ...
            return (finalizer ? statementNext(h, finalizer) : h);
        }) :
