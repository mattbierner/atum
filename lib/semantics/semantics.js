...
var _increment = function(expr) {
    return compute.bind(
        ...
        function(oldValue) {
            return reference_semantics.putValue(compute.always(expr), compute.always(number.add(oldValue, new number.Number(1))));
        });
};
    ...
    return compute.bind(
        ...
        function(baseValue) {
            return compute.bind(
                ...
                function(name) {
                    return function(ctx, ok, err) {
                        return ok(new reference.Reference(name, baseValue, ctx.strict), ctx);
                    };
                });
        });
};
