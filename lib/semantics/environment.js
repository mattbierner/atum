...
EnvironmentReference.prototype.dereference = function(ctx) {
    var name = this.name,
        strict = this.strict;
    return compute.bind(this.base.dereference(ctx), function(env) {
    ...
    return compute.bind(
        ...
        function(ctx) {
            return compute.bind(
                ...
                function(resolved) {
                    return resolved ?
                        compute.always(new EnvironmentReference(name, resolved)) :
                        compute.always(new reference.ValueReference(name, undefined));
                });
        });
};
...
var setMutableBinding = function(name, strict, value) {
    return compute.binda(
        compute.sequence(
            ...
            compute.context),
        function(v, ctx) {
            return compute.next(environment.setIdentifierReference(ctx, ctx.lexicalEnvironment, name, v, strict), compute.always(v));
        });
};
