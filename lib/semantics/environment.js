...
var pushEnvironment = function(f) {
    return compute.bind(getEnvironment(), function(env) {
        return compute.bind(iref.create(compute.always(f(env))), setEnvironment);
    });
};
...
var putIdentifierReference = function(ref, name, value) {
    return (!ref ?
        ...
        compute.bind(
            ref.getValue(),
            function(lex) {
                return ref.setValue(compute.always(
...
var setIdentifierReference = function(ref, name, value) {
    return compute.bind(ref.getValue(), function(lex) {
        return (lex.outer === null || environment_record.hasBinding(lex.record, name) ?
            ref.setValue(compute.always(environment.putIdentifier(lex, name, value))) :
