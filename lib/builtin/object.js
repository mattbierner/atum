...
ObjectPrototype.prototype.defaultValue = function(ref, hint) {
    var toString = value_reference_semantics.getValue(this.get(ref, "toString")),
        valueOf = value_reference_semantics.getValue(this.get(ref, "valueOf"));
    ...
    case 'String':
        return compute.bind(toString, function(toStringImpl) {
            if (toStringImpl && value.isCallable(toStringImpl)) {
                return toStringImpl.call(ref, []);
            }
            return compute.bind(valueOf, function(valueOfImpl) {
                return valueOf.call(ref, []);
            });
        });
    case 'Number':
    default:
