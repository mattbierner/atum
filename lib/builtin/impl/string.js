...
/* StringInstance
 ******************************************************************************/
var StringInstance = record.extend(meta_string.String,
    [],
    function(primitiveValue) {
        meta_string.String.call(this, this.proto, this.properties, true, primitiveValue);
    });
 ...
 * `new String([value])`
 */
var StringConstruct = compute.composen(
    function(ref, args) {
        return func.forward(ref, null, args);
    },
    fun.compose(
        value_reference.create,
        StringInstance.create));
...
var StringProperties = {
    'prototype': property.createValuePropertyFlags(
        string_builtin.StringPrototype)
};

/* StringPrototype
 ******************************************************************************/
var StringPrototype = new meta_object.Object(
            ...
            return value_reference.dereferenceFrom(
                value_reference.dereference(regExp, function(reg, regexpRef) {
                    return (value.isObject(reg) && reg.cls === 'RegExp' ?
                        ...
                        construct.construct(regexp_builtin.RegExp, [regexpRef]));
                }),
                function(reg) {
                            ...
                            return arr.reduce(function(p, c, i) {
                                ...
                                if (value.isObject(replace) && value.isCallable(replace)) {
                                    ...
                                    return string_op.concat(p, func.call(
                                        replacementRef,
                                        null,
                                        compute.eager(compute.enumeration(
                                            c.slice(m).map(string_op.create),
                                            number.create(c[m + 1]),
                                            string_op.create(c[m + 2])))));
                                }
...
var initialize = function() {
    return compute.sequence(
        builtin_constructor.create(string_builtin.String, 'String', 1, StringProperties, StringCall, StringConstruct),
