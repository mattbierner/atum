            ...
            return compute.bind(
                ...
                function(argArray) {
                    if (value.isUndefined(argArray) || value.isNull(argArray))
                        return func.call(compute.just(thisObj), compute.just(thisArg), compute.enumerationa(args.args.map(compute.just)));
                    ...
                    return compute.bind(
                        ...
                        function(len) {
                            return (function loop(i, args) {
                                return (i >= len.value ?
                                    func.call(compute.just(thisObj), compute.just(thisArg), compute.enumerationa(args.args.map(compute.just))) :
