...
var numberPrototypeToPrecision = function(ref, thisObj, args) {
    ...
    return compute.binary(
        ...
        function(t, precision) {
            if (precision.value < 1 || fractionDigits.value > 21)
                return error.rangeError();
            
            if (t instanceof NumberInstance)
                return number_operations.toPrecision(t.primitiveValue, fractionDigits.value);
            else if (value.isNumber(t))
                return number_operations.toPrecision(t, fractionDigits.value);
            return error.typeError();
        });
};
