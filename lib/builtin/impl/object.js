    ...
    return compute.bind(
        ...
        function(t) {
            ...
            return compute.binds(
                compute.enumeration(
                    object_semantics.get(self, 'enumerable'),
                    object_semantics.get(self, 'configurable'),
                    object_semantics.get(self, 'value'),
                    object_semantics.get(self, 'writable'),
                    object_semantics.get(self, 'get'),
                    object_semantics.get(self, 'set')),
                function(descEnumerable, descConfigurable, descValue, descWritable, descGet, descSet) {
                    if (!value.isUndefined(descGet) || !value.isUndefined(descSet)) {
                        if (!value.isUndefined(descValue) || !value.isUndefined(descWritable))
                            return error.typeError();
                    }
                    return compute.just({
                        'enumerable': (descEnumerable ),
                        'value': descValue,
    ...
    return compute.bind(
        ...
        function(t) {
            if (!(value.isNull(t) || value.isObject(t)))
                return error.typeError();
            return compute.bind(
                object_semantics.construct(compute.just(object_refs.Object), compute.enumeration()),
                function(obj) {
                    return (properties ?
                        objectDefineProperties(object_refs.ObjectDefineProperty, [args, properties]) :
                        compute.just(obj));
                });
        });
};
 ...
 */
var objectDefineProperties = function(ref, _, args) {
