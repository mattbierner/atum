                        ...
                        function(descriptors) {
                            return descriptors.reduce(function(p, pair) {
                                return compute.bind(p, function(ref) {
                                    return compute.bind(value_reference.getValue(compute.just(ref)), function(t){
...
var objectPrototypeIsPrototypeOf = (function(){
    var loop = function(v, o) {
        ...
        return compute.bind(
            value_reference.getValue(compute.just(v.proto)),
            function(v) {
                return (v === o ?
                    boolean.TRUE :
                    loop(v, o));
    ...
    return function(ref, self, args) {
        var v = args.getArg(0);
        return compute.bind(
            value_reference.getValue(compute.just(v)),
            function(v) {
                if (!value.isObject(v))
                    return boolean.FALSE;
                return compute.bind(
                    compute.bind(value_reference.getValue(compute.just(self)), type_conversion.toObject),
                    function(o) {
                        return loop(v, o);
                    });
            });
