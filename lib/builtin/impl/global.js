...
var globalEval = function(ref, thisBinding, args) {
    ...
    return (value.isString(x) ?
        evaluation.evaluateEvalText(x.value) :
        compute.just(x));
};
 ...
 * parseFloat(str)
 */
var globalParseFloat = function(ref, thisBinding, args) {
    var str = args.getArg(0);
    return compute.bind(
        type_conversion.toString(str),
        function(str) {
            var input = str.value.trim();
            return number_operations.create(hostParseFloat(input));
        });
};
...
var initializeGlobal = function() {
    return compute.sequence(
        ...
        builtin_function.create(global_builtin.globalParseFloat, 'parseFloat', 1, globalParseFloat));
};
...
var configureGlobal = function(mutableBinding) {
    return compute.sequence(
        ...
        mutableBinding('parseFloat', global_builtin.globalParseFloat));
};
