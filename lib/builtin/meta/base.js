...
Base.prototype.defaultValue = function(ref, hint) {
    ...
    case type.STRING:
        var vof = compute.bind(
            compute.bind(valueOf, function(f) { return func.functionCall(f, ref, compute.enumeration()); }),
            function(x) {
        ...
        return value_reference.dereferenceFrom(toString, function(toStringImpl, toStringRef) {
            return (toStringImpl && value.isCallable(toStringImpl) ?
                compute.bind(
                    func.functionCall(toStringRef, ref, compute.enumeration()),
                    function(x) {
                        return (x instanceof vr.ValueReference ?
                            vof :
                            compute.just(x))
    ...
    default:
        var vof = compute.bind(
            compute.bind(toString, function(f) { return func.functionCall(f, ref, compute.enumeration()); }),
            function(x) {
                return (x instanceof vr.ValueReference ?
                    error.typeError() :
                    compute.just(x));
        ...
        return value_reference.dereferenceFrom(valueOf, function(valueOfImpl, valueOfRef) {
            return (valueOfImpl && value.isCallable(valueOfImpl) ?
                compute.bind(
                    func.functionCall(valueOfRef, ref, compute.enumeration()),
                    function(x) {
                        return (x instanceof vr.ValueReference ?
                            vof :
                            compute.just(x))
    ...
    return compute.bind(this.getProperty(ref, name), function(prop) {
        if (prop) {
            if (property.hasGetter(prop))
                return func.functionCall(prop.get, ref, compute.enumeration());
            if (prop.value)
    ...
    return compute.bind(this.getProperty(ref, name), function(prop) {
        if (prop) {
            if (property.hasSetter(prop))
                return func.functionCall(prop.set, ref, compute.enumeration(compute.just(value)));
