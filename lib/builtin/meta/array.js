...
Array.prototype.defineProperty = function(ref, name, desc) {
    ...
    if (name === 'length' && oldLenDesc && oldLenDesc.value) {
        ...
        if (len >= oldLen)
            return self.defineOwnProperty(ref, name, newLenDesc);
        
        if (!oldLenDesc.writable)
            return error.rangeError();
        
        return compute.bind(self.defineOwnProperty(ref, name, newLenDesc), function(x) {
            var dels = [];
            for (var i = oldLen - 1; i > len; --i)
                dels.push(self.deleteProperty(ref, name));
