...
var globalEval = function(ref, thisBinding, args) {
    ...
    var ast;
    try {
        ast = parser.parseStream(lexer.lexRegExp(x.value));
    } catch (e) {
        return error.syntaxError(string.create(e + ""));
    }
    return compute.bind(execution_context_operations.getContext(), function(ctx) {
        return compute.next(execution_context_operations.createEvalContext(), //execution_context_operations.setContext(ctx),
            compute.bind(semantics.sourceElements(ast.body), function(x) {
