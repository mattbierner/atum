...
var globalEval = function(ref, thisBinding, args) {
    ...
    return compute.bind(execution_context_operations.getContext(), function(ctx) {
        return compute.between(execution_context_operations.setContext(evalCtx), execution_context_operations.setContext(ctx),
            compute.bind(semantics.mapSemantics(ast), function(x) {
                return compute.just(x);
            }));
    });
};
